document.addEventListener('DOMContentLoaded', () => {
    ...
    function changingBFS(i1, j1, original_Value){
        ...
        while (stack.length != 0){
            ...
            grid[x][y] = 0;
            size += 1; // //  
            if (x + 1 < grid.length && (grid[x + 1][y] == original_Value || (Math.abs(grid[x+1][y]-original_Value) == 6))) {
                stack.push([x + 1, y]);
            }
            if (x - 1 >= 0 && (grid[x - 1][y] == original_Value || (Math.abs(grid[x-1][y]-original_Value) == 6) )) {
                stack.push([x - 1, y]);
            }
            if (y + 1 < grid[x].length && (grid[x][y + 1] == original_Value || (Math.abs(grid[x][y+1]-original_Value) == 6))) {
                stack.push([x, y + 1]);
            }
            if (y - 1 >= 0 && (grid[x][y - 1] == original_Value || (Math.abs(grid[x][y-1]-original_Value) == 6) )) {
                stack.push([x, y - 1]);
            }
        }
        if (size >= 5){
    ...
    function dropSwapOrMove(ev) {
        ...
        if (sourceId - targetId == 1) {
            ...
            if (size >= 3){
                ...
                changingBFS(target_i, target_j, grid[target_i][target_j]);
                gravity();
                updateImages();
                printGridArray();
            }
